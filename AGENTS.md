...
- **Main game content**: `game/` (collections, game objects, scripts)
  - Current files: `game/game.collection`, `game/player.go`, `game/gltf.script`
- **Assets**: `assets/` (materials and 3D models)
  - Materials: `assets/materials/`
  - 3D models: `assets/models/` (`blue/`, `green/`, `neutral/`, `red/`, `yellow/`, `characters/`)
- **Dependencies (read-only context)**: `.deps/`
- **Texture profiles**: `all.texture_profiles`

Key Defold settings from `game.project`:

- **Bootstrap collection**: `/game/game.collectionc`
- **Physics type**: `3D`
...
- Current dependency state:
  - No `[project] dependencies#N` entries are configured in `game.project`.
  - `.deps/` currently contains Defold builtins (`.deps/builtins/`).
